...
const Item = ({
	...
	return (
		<Link href={`/city/${item.name}`} className="w-full">
			<div
				onClick={() => setSearch('')}
				className="text-[0.7rem] w-full text-foreground hover:opacity-50 px-[10px] py-[5px]"
			>
		...
		<div className="w-full max-w-[350px]">
			<label className="w-full flex-center">
				<input
					...
					className="t-input"
				/>
				<IconSearch className="text-foreground text-[20px] translate-x-[-25px]" />
			</label>
			{isLoading && (
			...
			{isError && (
				<span className="text-foreground text-[0.8rem] p-[10px] h-[20px] mt-[10px] absolute">
					{error.message}
				</span>
			)}
			{data && (
				<section className="w-[30%] min-w-[150px] max-w-[300px] bg-surface rounded absolute mt-[10px]">
					{data.map((item) => (
